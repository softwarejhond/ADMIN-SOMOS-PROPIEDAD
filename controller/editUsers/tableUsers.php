                    ...
                    <form id="updateForm<?php echo $user['id']; ?>">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="rol" required>
                                ...
                                <option value="2" <?php echo ($user['rol'] == 2) ? 'selected' : ''; ?>>Operario</option>
                                <option value="3" <?php echo ($user['rol'] == 3) ? 'selected' : ''; ?>>Aprobador</option>
                                <option value="4" <?php echo ($user['rol'] == 4) ? 'selected' : ''; ?>>Editor</option>
                                <option value="5" <?php echo ($user['rol'] == 5) ? 'selected' : ''; ?>>Arrendatario</option>
                            </select>
                        </div>
...
<script>
    ...
    function updateUser(userId) {
        ...
        $.ajax({
            url: 'controller/editUsers/update_user.php',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            error: function(xhr, status, error) {
                Swal.fire({
                    ...
                    text: 'HTTP ' + xhr.status + ': ' + (xhr.responseText || error || 'Sin respuesta del servidor')
                });
            }
        });
    }
    ...
    function initDeleteModal(userId) {
        ...
        $("#deleteBtn" + userId).click(function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: 'controller/editUsers/delete_user.php',
                        type: 'POST',
                        data: {
                            id: userId
                        },
