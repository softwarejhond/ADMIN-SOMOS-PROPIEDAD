    ...
    if ($fieldName == 'tipoInmueble') {
        ...
        if (mysqli_num_rows($resultTipo) > 0) {
            while ($row = mysqli_fetch_assoc($resultTipo)) {
                ...
                $selected = ($tipoNombre == $valor) ? 'selected' : '';
        ...
        foreach ($estratos as $value) {
            ...
            echo "<input type='radio' class='form-check-input' name='estrato' id='estrato_$value' value='$value' required $checked>";
        ...
        foreach ($opcionesParqueadero as $opcion) {
            ...
            echo "<input type='radio' class='form-check-input' name='$fieldName' id='parqueadero_$value' value='$value' required $checked>";
        ...
        foreach ($habitaciones as $value) {
             $checked = ($value == $valor) ? 'checked' : '';
            echo "<div class='form-check-inline'>";
            echo "<input type='radio' class='form-check-input' name='$fieldName' id='habitaciones_$value' value='$value' required  $checked>";
        ...
        foreach ($closetOpciones as $value) {
            $checked = ($value == $valor) ? 'checked' : '';
            echo "<div class='form-check-inline'>";
            echo "<input type='radio' class='form-check-input' name='$fieldName' id='closet_$value' value='$value' required $checked>";
        ...
        foreach ($cocina as $value) {
            $checked = ($value == $valor) ? 'checked' : '';
            echo "<div class='form-check-inline'>";
            echo "<input type='radio' class='form-check-input' name='$fieldName' id='cocina_$value' value='$value' required $checked>";
        ...
        foreach ($opcionesVista as $value) {
           $checked = ($value == $valor) ? 'checked' : '';
            echo "<div class='form-check-inline'>";
            echo "<input type='radio' class='form-check-input' name='$fieldName' id='vista_$value' value='$value' required  $checked>";
        ...
        foreach ($opcionesServicios as $value) {
            $checked = ($value == $valor) ? 'checked' : '';
            echo "<div class='form-check-inline'>";
            echo "<input type='radio' class='form-check-input' name='$fieldName' id='servicios_$value' value='$value' required $checked>";
...
<script>
    function updateCharacterCount() {
       ...
       echo "<label class='form-label text-magenta-dark'>" . ucfirst(str_replace('_', ' ', 'Teléfono inmueble')) . "</label>";
        ...
        echo "<input type='tel' id='telefono' name='$fieldName' class='form-control' placeholder='Escriba el número de teléfono' autocomplete='off' value='".htmlspecialchars($valor)."'>";
         ...
         // Campo para el valor del canon
            echo "<input type='number' id='valor_canon' name='$fieldName' class='form-control' placeholder='Ingrese el valor del canon' autocomplete='off' required value='".htmlspecialchars($valor)."'>";
          echo "</div>";
       ...
       // Campo para el documento del propietario
        echo "<input type='text' id='doc_propietario' name='$fieldName' class='form-control' placeholder='Ingrese el documento del propietario' autocomplete='off' required value='".htmlspecialchars($valor)."'>";
         echo "</div>";
         ...
         echo "<div class='form-group'>";
          ...
          // Campo para el nombre del propietario
           echo "<input type='text' id='nombre_propietario' name='$fieldName' class='form-control' placeholder='Ingrese el nombre del propietario' autocomplete='off' required value='".htmlspecialchars($valor)."'>";
        echo "</div>";
         ...
         echo "<div class='form-group'>";
          ...
          echo "<input type='tel' id='telefono_propietario' name='$fieldName' class='form-control' placeholder='Ingrese el teléfono del propietario' autocomplete='off' required value='".htmlspecialchars($valor)."'>";
        echo "</div>";
        ...
        echo "<div class='form-group'>";
         ...
         echo "<input type='email' id='email_propietario' name='$fieldName' class='form-control' placeholder='Ingrese el correo electrónico del propietario' autocomplete='off' required value='".htmlspecialchars($valor)."'>";
        echo "</div>";
       ...
       // Campo select para elegir el tipo de cuenta
        ...
        echo "<div class='form-group'>";
         ...
         // Campo para el número de cuenta
           echo "<input type='number' id='numeroCuenta' name='$fieldName' class='form-control' placeholder='Ingrese el número de cuenta' required value='".htmlspecialchars($valor)."'>";
        ...
        echo "<div class='form-group'>";
            echo "<label class='form-label text-magenta-dark'>" . ucfirst(str_replace('_', ' ', 'Fecha de ingreso')) . "</label>";
             ...
             // Input de fecha
            echo "<input type='date' id='fechaIngreso' name='$fieldName' class='form-control' required value='".htmlspecialchars($valor)."'>";
       ...
       echo "<div class='form-group'>";
        echo "<label class='form-label text-magenta-dark'>" . ucfirst(str_replace('_', ' ', 'Contrato EPM')) . "</label>";
         ...
         echo "<input type='text' id='contrato_EPM' name='$fieldName' class='form-control' placeholder='Ingrese el número de contrato EPM' required value='".htmlspecialchars($valor)."'>";
...
<script>
    // Script para carga de departamentos y municipios
    document.addEventListener('DOMContentLoaded', function() {
        const lista_departamento = document.getElementById('lista_departamento');
        const municipios_select = document.getElementById('municipios');
        const barrios_select = document.getElementById('barrios');

        // Función para cargar departamentos
        function loadDepartments() {
            fetch('../get_departments.php') // Asegúrate de que este archivo exista y funcione
                .then(response => response.json())
                .then(data => {
                    lista_departamento.innerHTML = '<option value="">Seleccionar</option>'; // Opción por defecto
                    data.forEach(department => {
                        const option = document.createElement('option');
                        option.value = department.departamento;
                        option.textContent = department.departamento;
                        lista_departamento.appendChild(option);
                    });
                })
        ...
        function loadMunicipalities(department) {
            if (department) {
                fetch('../get_municipalities.php?departamento=' + encodeURIComponent(department)) // Asegúrate de que este archivo exista y funcione
                    .then(response => response.json())
                    .then(data => {
                         municipios_select.innerHTML = '<option value="">Seleccionar</option>'; // Opción por defecto
                        data.forEach(municipality => {
                            const option = document.createElement('option');
                            option.value = municipality.municipio;
                            option.textContent = municipality.municipio;
                           municipios_select.appendChild(option);
                        });
                    })
                    .catch(error => console.error('Error fetching municipalities:', error));
            } else {
                municipios_select.innerHTML = '<option value="">Seleccionar departamento primero</option>';
            }
         }
            // Función para cargar barrios según el municipio seleccionado
        function loadBarrios(municipio) {
            if (municipio) {
                fetch('../get_barrios.php?municipio=' + encodeURIComponent(municipio)) // Asegúrate de que este archivo exista y funcione
                    .then(response => response.json())
                    .then(data => {
                         barrios_select.innerHTML = '<option value="">Seleccionar</option>'; // Opción por defecto
                        data.forEach(barrio => {
                            const option = document.createElement('option');
                            option.value = barrio.barrio;
                            option.textContent = barrio.barrio;
                            barrios_select.appendChild(option);
                        });
                    })
                    .catch(error => console.error('Error fetching barrios:', error));
            } else {
                 barrios_select.innerHTML = '<option value="">Seleccionar municipio primero</option>';
            }
        }
        ...
        lista_departamento.addEventListener('change', function() {
            const selectedDepartment = this.value;
            loadMunicipalities(selectedDepartment);
              municipios_select.value = "";
             barrios_select.value ="";
               
        });
            // Listener para cambios en el municipio
            municipios_select.addEventListener('change', function() {
                const selectedMunicipio = this.value;
                loadBarrios(selectedMunicipio);
                barrios_select.value ="";
            });
    });
</script>