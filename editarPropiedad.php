...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'actualizarRegistro') {
    echo "entro"; // Agrega esta línea para verificar la condicional
    echo "<pre>";
    var_dump($_POST);
    echo "</pre>";
    echo "<script>console.log('Valor de action:', " . json_encode($_POST['action']) . ");</script>";

    // Recoger los valores del $_POST y asignarlos a variables
    $codigo = $_POST['codigo'];
    ...
    $vista = $_POST['vista'];
    $servicios_publicos = $_POST['servicios_selected'];
    ...
    if (isset($_FILES['url_foto_principal']) && $_FILES['url_foto_principal']['error'] == 0) {
        $nombreArchivo = basename($_FILES['url_foto_principal']['name']);
        $ruta1 = 'fotos/' . $nombreArchivo; // Ruta completa del archivo
        if (!move_uploaded_file($_FILES['url_foto_principal']['tmp_name'], $ruta1)) {
            echo "<script>
        ...
        $datos['url_foto_principal'] = $nombreArchivo; // Guardar solo el nombre del archivo en la base de datos
    }
    echo "<script>console.log('Datos que se enviaran al actualizar:', " . json_encode($datos) . ");</script>";
    $resultado = actualizarRegistro($conn, $datos, $_GET['tabla']);

    if ($resultado['success'] === true) {
        echo "<script>console.log('Información actualizada:', " . json_encode($datos) . ");</script>";
         header("Location: index.php");
          exit;
    } else {
      echo "<script>console.log('Error:', " . $resultado['message'] . ");</script>";
    }
}
